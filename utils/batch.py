...
def get_queries(
    input_path: Union[str, Path], sort_queries_by: T.Optional[str] = "length"
) -> Tuple[List[Tuple[str, T.Union[str, List[str]], Optional[List[str]]]], bool]:
    ...
    queries: T.List[T.Tuple[str, T.Union[str, List[str]], T.Optional[T.List[str]]]]
    query_sequence: T.Union[str, List[str]]
    a3m_lines: T.Optional[List[str]]
    if input_path.is_file():
        if input_path.suffix == ".csv" or input_path.suffix == ".tsv":
            ...
            (seqs, headers) = parse_fasta(input_path.read_text())
            if len(seqs) == 0:
    ...
    for job_number, (raw_jobname, query_sequence, a3m_lines) in enumerate(queries):
        ...
        if a3m_lines is not None and a3m_lines[0].startswith("#"):
            ...
            if len(tab_sep_entries) == 2:
                s2i: T.Callable[[str], int] = lambda x: int(x)
                query_seq_len = list(map(s2i, tab_sep_entries[0].split(",")))
                query_seqs_cardinality = list(map(s2i, tab_sep_entries[1].split(",")))
                is_single_protein = (
                    True
...
def msa_to_str(
    ...
    paired_msa: T.Optional[List[str]],
    query_seqs_unique: List[str],
    query_seqs_cardinality: List[int],
) -> str:
